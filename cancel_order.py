...
import pandas as pd
from input import cancel_orders
...
def new_orders_func(check_orders ,position_side,p):
...
def flat_orders_func(check_orders ,hv_position, position_side,p):
...
def type_of_orders_cancel_func(check_orders,hv_position,position_side,p,order_type):

    print("find_out which type of orders do u need\n")
    new_orders = new_orders_func(check_orders ,position_side,p)
    print(f"new_orders:{new_orders}")
    flat_orders = flat_orders_func(check_orders ,hv_position, position_side,p)
...
def cancel_orders_func(order_id,trade_session):
    print("cancel_orders_func\n")
    for order_ids in order_id:
        print(f"order_ids:{order_ids}")
        trade_session.cancel_order(coin_symbol, order_ids)

def cancel_orders(coin_symbol, order_type, p):
        ...
        order_id = type_of_orders_cancel_func(check_orders,hv_position,position_side,p,order_type)
        print(f"order_id:{order_id}")
        if order_id != None:
            real_trade_data = cancel_orders_func(order_id,trade_session)

        return real_trade_data

if __name__ == "__main__":

    test_real, coin_symbol, order_type, \
    p, Accs = cancel_orders().execute_all()
    ...
    for acc_name, info in accs_info.items():
        ...
        trade_data = cancel_orders(coin_symbol, order_type, p)
